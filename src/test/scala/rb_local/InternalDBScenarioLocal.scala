...
class InternalDBScenarioLocal extends Simulation {

  val httpConf = http
    .baseUrl("https://gateway.aps2dev")

  val appName = "reproduce4"
  ...
  val scn = scenario("Basic interaction")
    ...
    .exec(http("startProcess")
      ...
      .body(StringBody(
        """
           {
               "processDefinitionId": "test:1:3dd52c4e-60f7-11e9-a6b2-96db893b2b3a",
    ...
    .exec(http("getProcessTask")
      .get("/v1/process-instances/${processInstanceId}/tasks")
      .header("Authorization", "Bearer ${token}")
      .check(jsonPath("$..id").ofType[String].saveAs("taskId"))
    )

    .exec(http("claimTask")
      .post("/v1/tasks/${standaloneTaskId}/claim")
      .queryParam("assignee", "devopsuser")
      .header("Authorization", "Bearer ${token}")
    )

    .exec(http("releaseTask")
      .post("/v1/tasks/${standaloneTaskId}/release")
      .queryParam("assignee", "devopsuser")
      .header("Authorization", "Bearer ${token}")
    )
